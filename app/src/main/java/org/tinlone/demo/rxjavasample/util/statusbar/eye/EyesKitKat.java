...
class EyesKitKat {
    ...
    static void setStatusBarImage(Activity activity,@DrawableRes int imgRes) {
        ...
        removeFakeStatusBarViewIfExist(activity);
        //添加一个View来作为状态栏的填充
        addImgStatusBarView(activity, imgRes, statusBarHeight);
        //设置子控件到状态栏的间距
        addMarginTopToContentChild(mContentChild, statusBarHeight);
        //不预留系统栏位置
        if (mContentChild != null) {
    ...
    private static void addImgStatusBarView(Activity activity, @DrawableRes int imgRes, int statusBarHeight) {
        Window window = activity.getWindow();
        ViewGroup mDecorView = (ViewGroup) window.getDecorView();

        ImageView mStatusBarView = new ImageView(activity);
        FrameLayout.LayoutParams layoutParams = new FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, statusBarHeight);
        layoutParams.gravity = Gravity.TOP;
        mStatusBarView.setLayoutParams(layoutParams);
        mStatusBarView.setScaleType(ImageView.ScaleType.FIT_XY);
        mStatusBarView.setImageResource(imgRes);
        mStatusBarView.setTag(TAG_FAKE_STATUS_BAR_VIEW);
